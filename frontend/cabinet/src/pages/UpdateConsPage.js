...
const UpdateConsulationPage = () => {
  const { cnp, data } = useParams();
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.patch(
        'http://127.0.0.1:8001/api/medical_office/consultation',
        formData,
        {
          headers: {
  ...
  return (
    <div>
      ...
      <Link to='/consultations/:id_doctor'>Pagina anterioara</Link>
    </div>
  );
};
export default UpdateConsulationPage;